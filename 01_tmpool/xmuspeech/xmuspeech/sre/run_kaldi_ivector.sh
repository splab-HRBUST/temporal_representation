...
train=data/mfcc_20_5.0/train_volume_aug_nosil
...
for data in $train $task1_test_3sec $task1_test_10sec $task1_test_30sec $task1_enroll; do
    subtools/createVisualVad.sh $data
done
...
subtools/kaldi/utils/subset_data_dir.sh $train 30000 ${train}_30k
...
subtools/kaldi/sid/train_diag_ubm.sh --nj $train_nj --cmd "run.pl" ${train}_30k ${cnum} $exp/diag_ubm_${cnum}
fi

if [[ $stage -le 2 && 2 -le $endstage ]];then 
echo "[step 2 Full UBM training"
subtools/kaldi/sid/train_full_ubm.sh --nj $train_nj --cmd "run.pl" ${train}_60k $exp/diag_ubm_${cnum} $exp/full_ubm_${cnum} 
fi
...
toEXdata="train_volume_aug_nosil lre07_test_3sec lre07_test_10sec loe07_test_30sec lre07_enroll_volume_aug"
nj=5
