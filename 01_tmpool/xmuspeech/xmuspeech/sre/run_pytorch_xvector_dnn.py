...
preprocess_nj = 20
cmn = False # Traditional cmn process.
...
overlap=0
scale=1 # Get max / num_spks * scale for every speaker.
# valid_split_type="--total-spk" # --total-spk or --default
valid_split_type="--default" # --total-spk or --default
valid_utts = 1024*2
valid_chunk_num_every_utt = 1
##--------------------------------------------------##
## Training options
...
model_params = {
    ...
    "simplenet_params": [512, 256],
    "fc_params": {"nonlinearity":'relu', "nonlinearity_params":{"inplace":True},
                "bn-relu":False, 
                "bn":True,  
...
traindata="data/resnet/train"
egs_dir="exp/egs_dnn/train" + "_" + sample_type + "_max"

# model_blueprint="subtools/pytorch/model/snowdar-xvector.py"
model_blueprint="subtools/pytorch/model/dnn-xvector.py"
model_dir="exp/pytorch_xvector_bilstm_mean_std"
##--------------------------------------------------##
##
######################################################### START #########################################################
##
...
if stage <= 3 <= endstage:
    ...
    model = model_py.FusionNetXvector(info["feat_dim"], info["num_targets"], **model_params)
...
if stage <= 4 <= endstage and utils.is_main_training():
    ...
    prefix = "resnet" # For to_extracted_data.
    ...
    # to_extracted_data = ["lre07_test_3sec","lre07_test_10sec","lre07_test_30sec","lre07_enroll_volume_aug"] # All dataset should be in data_root/prefix.
    # to_extracted_data = ["lre07_enroll_volume_aug"] # All dataset should be in data_root/prefix.
    to_extracted_data = ["train_nosil","task_1_nosil","task_2_enroll_nosil","task_2_nosil","task_3_test_nosil","task_3_enroll_nosil"]
    to_extracted_epochs = ["{}".format(epochs)] # It is model's name, such as 10.params or final.params (suffix is w.r.t package).
