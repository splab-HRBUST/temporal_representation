...
optimizer_params = {
    ...
    "beta2":0.999,
    "beta3":0.999,
    "weight_decay":0.,  # Should be large for decouped weight decay (adamW) and small for L2 regularization (sgd, adam).
    "lookahead.k":5,
    "lookahead.alpha":0.,  # 0 means not using lookahead and if used, suggest to set it as 0.5.
    "gc":False # If true, use gradient centralization.
}
...
report_interval_iters = 10 # About validation computation and loss reporting. If report_times_every_epoch is not None, 
...
# exist_model="exp/pytorch_xvector_fusion_E2E_backup/9.params" 
...
model_blueprint="subtools/pytorch/model/resnet-fusion_rank_stat-xvector.py"
model_dir="exp/pytorch_xvector_fusion_E2E_backup"
##--------------------------------------------------##
##
######################################################### START #########################################################
##
...
if stage <= 4 <= endstage and utils.is_main_training():
    ...
    to_extracted_data = ["train_nosil", "task_1_nosil", "task_2_nosil", "task_3_enroll_nosil", "task_3_test_nosil"]
    to_extracted_epochs = ["9"] # It is model's name, such as 10.params or final.params (suffix is w.r.t package).
