...
if [ $stage -eq 6 ]; then
  ...
  # python3.8 run_pytorch_xvector_resnet.py --stage=3 --gpu-id=2,3
  # python3.8 run_pytorch_xvector_resnet.py --stage=0
  ...
  subtools/runPytorchLauncher.sh run_pytorch_xvector_resnet_fusion_rank_stat.py --gpu-id=0,1 --multi-gpu-solution="ddp"
...
if [ $stage -eq 7 ]; then
  ...
  # durs="3sec" 
  for exp in exp/pytorch_xvector/far_epoch_21;do
  # for exp in exp/pytorch_xvector/near_epoch_21;do
    for dur in $durs;do
    # subtools/scoreSets.sh --eval false --vectordir $exp --prefix mfcc_20_5.0   --enrollset lre07_enroll_volume_aug --testset lre07_test_${dur} \
    #                       --lda true --clda 100 --submean true --score "lr" --metric "eer"
    subtools/scoreSets.sh --eval false --vectordir $exp --prefix resnet   --enrollset lre07_enroll_volume_aug --testset lre07_test_${dur} \
                          --lda true --clda 100 --submean true --score "cosine" --metric "eer"
    # subtools/scoreSets.sh --eval false --vectordir $exp --prefix fbank_40_5.0   --enrollset lre07_enroll_volume_aug --testset lre07_test_${dur} \
    #                       --lda true --clda 100 --submean true --score "lr" --metric "eer"
  done
  done
fi
