...
class ChunkEgs(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.aug is not None:
            ...
            # return egs.T, target
            # print("=========================", np.expand_dims(egs.T, 1).shape)
            return np.expand_dims(egs.T, 1), target
            if len(egs.T.shape) == 3:
                return egs.T, target
            elif len(egs.T.shape) == 2:
                print("========================",egs.T.shape)
                return np.expand_dims(egs.T, 2), target
