...
class RankPooling(torch.nn.Module):
    ...
    def smooth(self, inputs):
        inputs_ = inputs
        temp_tensor_ = torch.Tensor(inputs_.shape[0],inputs_.shape[1], inputs_.shape[2])
        for samples_index in range(0, inputs_.shape[0]):
            ...
            #=======================================================
            temp_tensor_[samples_index] = frames_
        return temp_tensor_

    def norm_smooth(self, inputs):
        inputs_ = inputs
        temp_tensor_ = torch.Tensor(inputs_.shape[0],inputs_.shape[1], inputs_.shape[2])
        for samples_index in range(0, inputs_.shape[0]):
            frames_ = inputs_[samples_index]
            ...
            pre_norms = torch.norm(frames_, p=2, dim=0, keepdim=True)
            frames_ = frames_ / pre_norms
            #=======================
            frames_ = frames_.cumsum(1)
            frames_ = frames_ / (torch.Tensor(range(0, frames_.shape[1])) + 1)
            # normalize
    ...
    def nonlinearize(self, inputs):
        inputs_ = inputs
        temp_tensor_ = torch.Tensor(inputs_.shape[0],inputs_.shape[1]*2, inputs_.shape[2])
        for samples_index in range(0, inputs_.shape[0]):
            frames_ = inputs_[samples_index]
            # nonlinear
            #==============================================
            ...
            # print("frames_pos = {}".format(frames_pos))
            # print("frames_neg = {}".format(frames_neg))
            ...
            temp_tensor_[samples_index] = ret_frames
    ...
    def get_model_params(self, inputs):
        inputs_ = inputs
        temp_tensor_ = torch.Tensor(inputs_.shape[0],inputs_.shape[1], 1)
        ...
        for samples_index in range(0, inputs_.shape[0]):
            ...
            svr = SVR(kernel='linear', C = 1e2, cache_size = 2048, tol = 1e-3, max_iter = -1)
            ...
            if X.shape[1] % 41 == 0:
                print("[R^2 score = {}]".format(svr.score(X.T,Y)))

            # temp_tensor_[samples_index] = torch.cat((w, w), dim=0)
            temp_tensor_[samples_index] = w
    ...
    def forward(self, inputs):
        ...
        # w = self.get_model_params(self.nonlinearize((self.norm_smooth(inputs))))
        w = self.get_model_params(self.norm_smooth(inputs))
        ...
        #================================================================
        # rank pooling var
        # counts = smoothed_inputs.shape[2]
        # w_var = torch.sum((smoothed_inputs - w)**2, dim=2, keepdim=True) / counts
        # w_std = torch.sqrt(w_var.clamp(min=1e-8))
        # return torch.cat((torch.cat((mean, std), dim=1), w), dim = 1)
        #================================================================
...
class RankPoolingPlus(torch.nn.Module):
    ...
    def get_model_params(self, inputs):
        ...
        for samples_index in range(0, inputs_.shape[0]):
            ...
            svr = SVR(kernel='linear', C = self.regu_c, cache_size = 2048, tol = 1e-3, max_iter = -1)
            # svr = SVR(kernel='linear', C = 100)
            svr.fit(X.T, Y.ravel())

            W = svr.coef_[0]
            W_norm = np.linalg.norm(x=W, ord=2)
            # print("W0 = {}, norm = {}".format(W[0], W_norm))
            W = W / W_norm
            W = W.reshape((768,1))
            b = svr.intercept_
