...
class ResNet(nn.Module):
    ...
    def _forward_impl(self, x):
        ...
        # print("raw x padding shape = {}".format(x.shape))
        if self.head_conv:
            x = self.conv1(x)
            x = self.bn1(x)
            x = self.relu(x)
        # print("after head_conv shape = {}".format(x.shape))
        
        if self.head_maxpool:
            x = self.maxpool(x)
        
        # print("pre layer1 x shape = {}".format(x.shape))
        x = self.layer1(x)
        # print("pre layer2 x shape = {}".format(x.shape))
        x = self.layer2(x)
        # print("pre layer3 x shape = {}".format(x.shape))
        x = self.layer3(x)
        # print("pre layer4 x shape = {}".format(x.shape))
        x = self.layer4(x)
        # print("after layer4 x shape = {}".format(x.shape))
