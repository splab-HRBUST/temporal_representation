...
def forward_proc(X, y, C, eps):
    n, dim = X.shape
    svr = SVR(kernel = 'linear', C = C, epsilon = eps, tol = 1e-3, max_iter = 1000000, verbose = False)
    # svr = LinearSVR(epsilon=0.0, tol=1e-4, C=1000, loss='squared_epsilon_insensitive', fit_intercept=False, random_state=0, max_iter=1000000)
    ...
    for i in range(len(svr.support_)):
        sv_travse[svr.support_[i]] = 1.0

    return svr.coef_[0].reshape(dim, 1), sv_travse.reshape(n, 1)
