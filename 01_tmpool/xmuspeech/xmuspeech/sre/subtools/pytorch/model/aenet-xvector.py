...
class AeNetXvector(TopVirtualNnet):
    """ A aenet x-vector framework """
    
    def init(self, inputs_dim, num_targets, aug_dropout=0., tail_dropout=0., training=True, extracted_embedding="near", 
             aenet_params={}, pooling="statistics", pooling_params={}, fc1=False, fc1_params={}, fc2_params={}, margin_loss=False, margin_loss_params={},
             use_step=False, step_params={}, transfer_from="softmax_loss"):

        ## Params.
        default_aenet_params = {
        ...
        aenet_params = utils.assign_params_dict(default_aenet_params, aenet_params)
        ...
        self.convXd = aenet_params["convXd"]
        ...
        self.aenet = AeNet(inplanes, **aenet_params)

        # It is just equal to Ceil function.
        aenet_output_dim = (inputs_dim + self.aenet.get_downsample_multiple() - 1) // self.aenet.get_downsample_multiple() \
                            * self.aenet.get_output_planes() if self.convXd == 2 else self.aenet.get_output_planes()

        # Pooling
        stddev = pooling_params.pop("stddev")
        if pooling == "lde":
            self.stats = LDEPooling(aenet_output_dim, c_num=pooling_params["num_head"])
        elif pooling == "attentive":
            self.stats = AttentiveStatisticsPooling(aenet_output_dim, hidden_size=pooling_params["hidden_size"], 
                                                    context=pooling_params["context"], stddev=stddev)
        elif pooling == "multi-head":
            self.stats = MultiHeadAttentionPooling(aenet_output_dim, stddev=stddev, **pooling_params)
        elif pooling == "multi-resolution":
            self.stats = MultiResolutionMultiHeadAttentionPooling(aenet_output_dim, **pooling_params)
        else:
            self.stats = StatisticsPooling(aenet_output_dim, stddev=stddev)

        self.fc1 = ReluBatchNormTdnnLayer(self.stats.get_output_dim(), aenet_params["planes"][3], **fc1_params) if fc1 else None

        if fc1:
            fc2_in_dim = aenet_params["planes"][3]
        else:
            fc2_in_dim = self.stats.get_output_dim()

        self.fc2 = ReluBatchNormTdnnLayer(fc2_in_dim, aenet_params["planes"][3], **fc2_params)
        ...
        if training :
            if margin_loss:
                self.loss = MarginSoftmaxLoss(aenet_params["planes"][3], num_targets, **margin_loss_params)
            else:
                self.loss = SoftmaxLoss(aenet_params["planes"][3], num_targets)

            # An example to using transform-learning without initializing loss.affine parameters
            self.transform_keys = ["aenet", "stats", "fc1", "fc2"]
    ...
    def extract_embedding(self, inputs):
        ...
        x = inputs
        # Tensor shape is not modified in libs.nnet.aenet.py for calling free, such as using this framework in cv.
        x = x.unsqueeze(1) if self.convXd == 2 else x
        x = self.aenet(x)
        x = x.reshape(x.shape[0], x.shape[1]*x.shape[2], x.shape[3]) if self.convXd == 2 else x
        x = self.stats(x)
...
if __name__ == "__main__":
    ...
    print("Test aenet2d ...")
    aenet2d = AeNetXvector(40, 1211, aenet_params={"convXd":2})
    print(aenet2d)
    print(aenet2d(tensor).shape)
    print("\n")
    print("Test aenet1d ...")
    aenet1d = AeNetXvector(40, 1211, aenet_params={"convXd":1})
    print(aenet1d)
    print(aenet1d(tensor).shape)
