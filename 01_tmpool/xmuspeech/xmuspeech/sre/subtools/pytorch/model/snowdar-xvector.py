...
class Xvector(TopVirtualNnet):
    ...
    def init(self, inputs_dim, num_targets, extend=False, skip_connection=False, 
        ...
        if pooling == "lde":
            ...
            print("----------------rank pooling-------------------")
            self.stats = RankPooling(num_nodes)
        else:
            print("------------------StatisticsPooling-------------------{}")
            self.stats = StatisticsPooling(num_nodes, stddev=stddev)

        stats_dim = self.stats.get_output_dim()
        print("-----------stats_dim = {}---------------".format(stats_dim))
        # Segment level
        if tdnn6:
    ...
    def forward(self, inputs):
        ...
        if self.skip_connection:
            x = x + identity
        x = self.tdnn5(x)
        print("tdnn5 output shape : {}".format(x.shape))

        x = self.stats(x)

        print("stats output shape : {}".format(x.shape))
        x = self.auto(self.tdnn6, x)
        print("tdnn6 output shape : {}".format(x.shape))
        x = self.tdnn7(x)
        print("tdnn7 output shape : {}".format(x.shape))
        x = self.auto(self.hidden_dropout, x)
        print("hidden_dropout output shape : {}".format(x.shape))
        return x
    ...
    def extract_embedding(self, inputs):
        ...
        # print("--------------{}---------------".format(extracted_embedding))
