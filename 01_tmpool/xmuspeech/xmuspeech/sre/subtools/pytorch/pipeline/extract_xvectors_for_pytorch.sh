...
if [ "$cmn" == "true" ];then
    ...
    if [ "$vad" == "true" ];then
        ...
        feats="ark:copy-vector scp:${sdata}/feats.scp ark:- |"
    fi
fi
...
if [ $stage -le 1 ]; then
      ...
      if $use_gpu; then
        ...
        for g in $(seq $nj); do
          $cmd --gpu 1 ${dir}/log/extract.$g.log \
            python3.8 subtools/pytorch/pipeline/onestep/extract_embeddings.py --use-gpu=$use_gpu --gpu-id="$gpu_id" \
      ...
      $cmd JOB=1:$nj ${dir}/log/extract.JOB.log \
          python3.8 subtools/pytorch/pipeline/onestep/extract_embeddings.py --use-gpu="false" \
                  --nnet-config=$srcdir/$nnet_config \
