...
if [ $num_gpu -gt 1 ];then
    if [ "$multi_gpu_solution" == "horovod" ];then
        ...
        train_cmd="horovodrun -np $num_gpu python3.8"
    elif [ "$multi_gpu_solution" == "ddp" ];then
        export OMP_NUM_THREADS=$omp_num_threads
        if [ "$port" == "0" ];then
            port=$(python3.8 subtools/pytorch/launcher/multi_gpu/get_free_port.py)
            launcher_options="$launcher_options --port $port"
        fi
        train_cmd="python3.8 -m torch.distributed.launch --nproc_per_node=$num_gpu"
    else
...
if [[ "$stage" -le 4 && "$endstage" -ge 4 ]];then
    python3.8 $launcher --stage=4 || exit 1
fi
