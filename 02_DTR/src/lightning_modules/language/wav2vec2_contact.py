...
from src.layers.mappling import cnn_mapping,fc_mapping,fc_relu_fc_mapping,fc_tanh_fc_mapping
...
# )
from src.lightning_modules.language.language_recognition_module import(
...
@dataclass
class Wav2vec2ContactModuleConfig:
...
class Wav2vec2ContactModule(LanguageRecognitionLightningModule):
    def __init__(
        self,
        hyperparameters_to_save: DictConfig,
        cfg: Wav2vec2ContactModuleConfig,
        num_speakers: int,
        ...
        # self.contact= DenseLayer(12*768,768,config_str='relu')
        self.cnn_mapping = cnn_mapping(768,1)
        # self.fc_mapping=fc_mapping(768,1)
        # self.frf_mapping=fc_relu_fc_mapping(768,384)
        # self.ftf_mapping=fc_tanh_fc_mapping(768,48)
        # create masker
        # self.embedding_masker = EmbeddingMasker(
        #     timestep_mask_prob=0.1,
        ...
        self.feature_dimension = self._get_wav2vec2_embedding_size()
        ...
        self.feature_layer=DenseLayer(self.stat_pool_dimension,self.feature_dimension,config_str='batchnorm_')
        # create fc layers
        self.fc_list = nn.ModuleList(
            [
                nn.Sequential(
                    nn.Linear(
                        in_features=self.feature_dimension
                        if idx == 0
    ...
    def _determine_embedding_size(self):
        if self.cfg.embedding_layer_idx < 0:
            ...
            return self._get_wav2vec2_embedding_size()
    ...
    def _fc_head_ops_pre_spk_embedding(self, wav2vec_embedding: t.tensor):
        ...
        masked_pooled_embedding = pooled_embedding #t.squeeze(
    ...
    def compute_speaker_embedding(self, input_tensor: t.Tensor) -> t.Tensor:
        ...
        wav2vec_embeddings,contact_embeddings = self.wav2vec(input_tensor)
        #mapping = cnn_mapping(768,2)
        #print(self.mapping.device)

        # wav2vec_embedding = self.contact(contact_embeddings)
        wav2vec_embedding = self.cnn_mapping(wav2vec_embeddings)
        wav2vec_embedding = t.transpose(wav2vec_embeddings, 2, 1)

        # wav2vec_embedding =self.fc_mapping(wav2vec_embeddings)
        # wav2vec_embedding =self.frf_mapping(wav2vec_embedding)
        # wav2vec_embedding =self.ftf_mapping(wav2vec_embedding)
        if self.training:
            pooled_embedding = self.stat_pooling(wav2vec_embedding)
        else:
            pooled_embedding = self.test_stat_pooling(wav2vec_embedding)

        # detach_embedding = self.mapping(wav2vec_embeddings.detach())

        # then we need to transpose to the expected input shape of
        # [BS, NUM_WINDOWS, NUM_FEATURES] by the rest of the network
        # out = t.transpose(wav2vec_embeddings, 2, 1)
        # out2 = t.transpose(wav2vec_embeddings, 2, 1)
        #print(out2.shape)
        # we end with all the operations to get to the speaker embeddings
        #return t.cat([pooled_embedding,detach_embedding],1),detach_embedding,wav2vec_embeddings.detach()
        
        # assert equal batch size, correct dimension and shape of len 2
        pooled_embedding = self.feature_layer(pooled_embedding)
        return pooled_embedding,pooled_embedding,wav2vec_embeddings.detach()
