...
from src.layers.mappling import cnn_mapping,fc_mapping
...
class Wav2vec2FCModule(SpeakerRecognitionLightningModule):
    def __init__(
        ...
        #self.mapping = cnn_mapping(768,1)
        # self.cnn_list =nn.ModuleList([nn.Conv1d(
        #                 in_channels=self._get_wav2vec2_embedding_size(),
        #                 out_channels=self._get_wav2vec2_embedding_size(),
        #                 kernel_size=5,
        #                 stride=1,
        #                 padding=2
        #             ) for i in range(2)
    ...
    def compute_speaker_embedding(self, input_tensor: t.Tensor) -> t.Tensor:
        ...
        #print(wav2vec_embeddings.dtype)
        #mapping = cnn_mapping(768,2)
        #print(self.mapping.device)
        
        # out =self.mapping(wav2vec_embeddings)
        # out2 = self.mapping(wav2vec_embeddings.detach())
        ...
        return self._fc_head_ops_pre_spk_embedding(out),self._fc_head_ops_pre_spk_embedding(out2),wav2vec_embeddings.detach()
