...
def load_base_wav2vec2_model(
    ...
    """
    huggingface_id="/g813_u1/g813_u2/w2v2-speaker-master/wav2vec2-base"
...
def wav2vec2_embed_raw_audio(input_tensor: t.Tensor, model: Wav2Vec2Model) -> t.Tensor:
    ...
    """
    output: Wav2Vec2BaseModelOutput = model(input_tensor,output_hidden_states=True)

    features = output.last_hidden_state
    hidden_state=output.hidden_states
    hidden_state=hidden_state[1:]
    contact_feature = t.cat(hidden_state,dim=-1)
    contact_feature=contact_feature.transpose(1,2)
    features = features.transpose(1, 2)
    # print(f'12312241234,{features.shape}')
    return features,contact_feature
...
class Wav2Vec2WrapperModule(pl.LightningModule):
    ...
    def forward(self, wav_input: t.Tensor):
        ...
        # else:
        #    embedding, contact_embedding= wav2vec2_embed_raw_audio(wav_input, self.model)
        embedding, contact_embedding= wav2vec2_embed_raw_audio(wav_input, self.model)
        
        # return an embedding with shape [BATCH_SIZE, NUM_FEATURES, NUM_FRAMES]
        return embedding,contact_embedding
