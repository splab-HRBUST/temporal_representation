...
from typing import Callable, Optional, List, Any, Tuple, Set,Dict
...
lt = 0.0001
#lt = 0.001
class LanguageRecognitionLightningModule(BaseLightningModule):
    def __init__(
        self,
    ...
    def training_step(
        self,
        batch: LanguageClassificationDataBatch,
        batch_idx: int,
    ...
    def _train_step_ce_loss(
        ...
        loss +=lt*self.lt_loss(embedding2,wav2vec_embeddings) 
    ...
    def validation_step(
        ...
        return {"embedding": embedding.detach().to("cpu"), "sample_id": sample_id, "ground_truth":label.detach().to("cpu")}
    ...
    def validation_epoch_end(self, outputs: List[Any]) -> None:
        ...
        if len(self.embeddings_queue) >= self.evaluator.max_num_training_samples:
            results = self._evaluate_embeddings(outputs)
    ...
    def test_epoch_end(self, outputs: List[Any]) -> None:
        self.log_dict(self._evaluate_embeddings(outputs))

    def _evaluate_embeddings(self, outputs: List[dict]):
        ...
        embedding_list,truth_list = self._extract_embedding_sample_list(outputs)        
        ...
        embedding_mean_dict= self.evaluator.caculate_embeding_mean(embedding_list)
        result = self.evaluator.evaluate1(embedding_mean_dict, embedding_list,truth_list)
            ...
            with t.no_grad():
                ...
                with tqdm.tqdm(
                    ...
                    for batch in self.train_dataloader():
                        ...
                        batch: LanguageClassificationDataBatch = batch
    ...
    def _extract_embedding_sample_list(outputs: List[dict]):
        ...
        for d in outputs:
            ...
            ground_truth_list=d["ground_truth"].tolist()

            if not isinstance(ground_truth_list, list):
                temp=[]
                temp.append(ground_truth_list)
                ground_truth_list=temp
            ...
            for idx, sample_id in enumerate(sample_id_list):
                if isinstance(embedding_tensor, list):
                    ...
                    if(ground_truth_list[idx] in ground_truth_dict.keys()):
                        assert sample_id.split("/")[0]==ground_truth_dict[ground_truth_list[idx]]
                    else:
                        ground_truth_dict[ground_truth_list[idx]]=sample_id.split("/")[0]
                    # all_ground_truth.add(ground_truth_list[idx])
                else:
                    embedding_list.append(
                        EmbeddingSample(
                            sample_id=sample_id,
        ...
        return embedding_list,ground_truth_dict
