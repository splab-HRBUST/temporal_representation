...
from src.data.modules.Language.OLR2020 import(
    OLR2020DataModuleConfig,
    OLR2020DataModule,
)
...
from src.lightning_modules.language import(
    Wav2vec2ContactModule,
    Wav2vec2ContactModuleConfig,
    LanguageRecognitionLightningModule,
)
...
def construct_data_module(
    cfg: DictConfig,
    test_path: pathlib.Path=pathlib.Path(""),
    ...
    elif isinstance(dm_cfg,OLR2020DataModuleConfig):
        if not isinstance(dl_cfg,languageDataLoaderConfig):
            raise ValueError(
                f"OLR2020DataModule expects {languageDataLoaderConfig},"
                f" got {dl_cfg}"
            )
        dm = OLR2020DataModule(
            cfg=dm_cfg,
            shard_cfg=shard_cfg,
            dl_cfg=dl_cfg,
            train_pipeline=train_pipeline,
...
def construct_speaker_recognition_module(
    ...
    dm: Union[SpeakerLightningDataModule,LanguageLightningDataModule],
...
def construct_language_recognition_module(
    cfg: DictConfig,
    network_cfg: DictConfig,
    evaluator: SpeakerRecognitionEvaluator,
    dm: Union[SpeakerLightningDataModule,LanguageLightningDataModule],
    ...
    # get init function based on config type
    if isinstance(network_cfg, Wav2vec2ContactModuleConfig):
        network_class = Wav2vec2ContactModule
    else:
        raise ValueError(f"cannot load network from {network_cfg}")
...
def construct_module(
    cfg: DictConfig,
    evaluator: SpeakerRecognitionEvaluator,
    dm: Union[SpeakerLightningDataModule, SpeechLightningDataModule,LanguageLightningDataModule],
    ...
    #         cfg, network_cfg, evaluator, dm, loss_fn_constructor
    #     )
    elif isinstance(dm,LanguageLightningDataModule):
        network = construct_language_recognition_module(
            cfg, network_cfg, evaluator, dm, loss_fn_constructor
        )
    else:
        raise ValueError(
...
def run_train_eval_script(cfg: DictConfig):
    ...
    dm = construct_data_module(cfg,test_path=pathlib.Path("/g813_u1/mnt/g813_u2/shards/test/task1"))
    ...
    #    profiler=profiler,
    # )
    dm2 = construct_data_module(cfg,test_path=pathlib.Path("/g813_u1/mnt/g813_u2/shards/test/task2"))
    dm3 = construct_data_module(cfg,test_path=pathlib.Path("/g813_u1/mnt/g813_u2/shards/test/task3"))
    ...
    if cfg.eval_model and cfg.fit_model:
        ...
        try:
            result = trainer.test(verbose=False, datamodule=dm)
            result2 = trainer.test(verbose=False, datamodule=dm2)
            result3 = trainer.test(verbose=False, datamodule=dm3)
            pass
        except:
            # there might not have been a validation epoch
            print("ther not have been a best weight")
            result = None
            result2 = None
            result3 = None
            pass
        result4 = trainer.test(ckpt_path="lightning_logs/version_0/checkpoints/last.ckpt",verbose=False, datamodule=dm)
        result5 = trainer.test(ckpt_path="lightning_logs/version_0/checkpoints/last.ckpt",verbose=False,datamodule=dm2)
        result6 = trainer.test(ckpt_path="lightning_logs/version_0/checkpoints/last.ckpt",verbose=False, datamodule=dm3)
        if(result is not None):
            print("this is the best weight's result")
            print("task1: ",end="")
            print(result[0])
            print("task2: ",end="")
            print(result2[0])
            print("task3: ",end="")
            print(result3[0])
        print("this is the newest weight's result")
        print("task1: ",end="")
        print(result4[0])
        print("task2: ",end="")
        print(result5[0])
        ...
        result = trainer.test(module,ckpt_path=None,verbose=False, datamodule=dm)
        result2 = trainer.test(module,ckpt_path=None,verbose=False, datamodule=dm2)
        result3 = trainer.test(module,ckpt_path=None,verbose=False, datamodule=dm3)
        print("task1: ",end="")
        print(result[0])
        print("task2: ",end="")
        print(result2[0])
